...
${PWD_XML}
    ...
    <!-- Shared PKCS#1 1024-bit RSA private key. Futu proto encryption is
         a shared-secret style scheme, not asymmetric — the same private
         key file is loaded by OpenD here and by every SDK client via
         SysConfig.set_init_rsa_file. Required whenever OpenD binds to a
         non-loopback address (we listen on 0.0.0.0 for the docker bridge
         network). Generated by entrypoint.sh on first boot; lives in a
         dedicated volume so it survives OpenD upgrades. -->
    <rsa_private_key>/rsa/rsa_private.pem</rsa_private_key>
</futu_opend>
XML
